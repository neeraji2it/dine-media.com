...
var Pages = {
  ...
  setActiveClass: function(page) {
    ...
    if (element) {
      ...
      if (group) {
        $('.zoom-img[data-strip-group="' + group + '"]').addClass('strip-active-group');
      }
    }
  },
...
var Window = {
    ...
    $(document.body).append(this.element = $('<div>').addClass('strp-window strp-measured')
      ...
      .append(this._previous = $('<div>').addClass('strp-nav strp-nav-previous')
        .append($('<div>').addClass('strp-nav-button')
          ...
          .append($('<i>').addClass('icon-16 font-3x'))
        )
        .hide()
      )
      ...
      .append(this._next = $('<div>').addClass('strp-nav strp-nav-next')
        .append($('<div>').addClass('strp-nav-button')
          .append($('<div>').addClass('strp-nav-button-background'))
          .append($('<div>').addClass('strp-nav-button-icon'))
          .append($('<i>').addClass('icon-17 font-3x'))
        )
        .hide()
      )
  ...
  _delegateHideOutside: function(event) {
    ...
    if (!$(element).closest('.zoom-img, .strp-window')[0]) {
      this.hide();
    }
  },
...
var _Strip = {
  ...
  startDelegating: function() {
    this.stopDelegating();
    $(document.documentElement).delegate('.zoom-img[href]', 'click', this._delegateHandler = $.proxy(this.delegate, this));
  },

  stopDelegating: function() {
    if (this._delegateHandler) {
      $(document.documentElement).undelegate('.zoom-img[href]', 'click', this._delegateHandler);
    ...
    switch ((object_type = $.type(object))) {
      ...
      case 'object':
        ...
        if (view.group) {
          ...
          if (isElement) {
            var elements = $('.zoom-img[data-strip-group="' + $(object).data('strip-group') + '"]');
